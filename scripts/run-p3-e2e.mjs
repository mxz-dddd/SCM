...
const databaseName = 'scm_p3_e2e';
const jwtSecret = 'scm-p3-e2e-jwt-secret-at-least-32-characters';
const seedPassword = 'scm-p3-e2e-admin-password';
...
let databaseUrl = process.env.P3_E2E_DATABASE_URL;
...
try {
  ...
  run(
    'pnpm',
    ['exec', 'playwright', 'test', 'apps/api/e2e/p3.acceptance.spec.ts'],
    environment,
  );
} finally {
  if (managedDatabase)
