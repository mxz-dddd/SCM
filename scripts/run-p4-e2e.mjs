...
const databaseName = 'scm_p4_e2e';
const jwtSecret = 'scm-p4-e2e-jwt-secret-at-least-32-characters';
const seedPassword = 'scm-p4-e2e-admin-password';
...
let databaseUrl = process.env.P4_E2E_DATABASE_URL;
...
try {
  ...
  run(
    'pnpm',
    ['exec', 'playwright', 'test', 'apps/api/e2e/p4.acceptance.spec.ts'],
    environment,
  );
} finally {
  if (managedDatabase)
