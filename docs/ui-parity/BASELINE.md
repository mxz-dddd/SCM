...
- 本轮可审计补证起点 HEAD：`6f242491bc964ea1a3924400f8e53ad30c543365`；相对 `main` 已有 19 个提交（13 个 V2 运行时、6 个 UI 对标）。
- Node.js：`v24.16.0`
- pnpm：`11.9.0`
...
- 新增类型化 `ADMIN_NAV_CATEGORIES`，管理端路由全部且仅归属一个一级大类和二级业务组；精确数量由路由注册测试动态断言，不在文档中固化旧数字。
...
- 参考侧取证包含仓储 188、订单 60、运输 86、基础资料 98、报表 79、消息 6，共 517 个业务叶子；这是参考证据覆盖，不是 517 个本地功能页面。
...
- 严格比较当前为第 1 轮同口径结果：22 个本地路由均输出几何、密度、字体、颜色关系和遮罩后图像差异。由于没有上一轮同口径分数，不宣称视觉收敛。
- 没有复制参考站 HTML、CSS、图标、图片、字体、品牌或业务数据。
