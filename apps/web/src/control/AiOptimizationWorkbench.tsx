...
const actions = createActionRegistry<string>([
  {
    id: 'refresh',
    label: '刷新 AI 工作台',
    requiredPermissions: ['control.ai.read'],
  },
  {
    id: 'route',
    label: '提交车辆路径优化',
    requiredPermissions: ['control.ai.route.optimize'],
  },
  {
    id: 'load',
    label: '提交装载优化',
    requiredPermissions: ['control.ai.load.optimize'],
  },
  {
    allowedStatuses: ['PROPOSED'],
    id: 'confirmLoad',
    label: '人工确认装载建议',
    requiredPermissions: ['control.ai.load.confirm'],
  },
  {
    allowedStatuses: ['CONFIRMED'],
    id: 'feedbackLoad',
    label: '反馈实际装载偏差',
    requiredPermissions: ['control.ai.load.feedback'],
  },
  {
    id: 'forecast',
    label: '生成需求预测版本',
    requiredPermissions: ['control.ai.forecast.manage'],
  },
  {
    allowedStatuses: ['DRAFT'],
    id: 'publishForecast',
    label: '发布预测版本',
    requiredPermissions: ['control.ai.forecast.publish'],
  },
  {
    allowedStatuses: ['PUBLISHED'],
    id: 'recommend',
    label: '生成补货建议',
    requiredPermissions: ['control.ai.inventory.recommend'],
  },
  {
    id: 'scenario',
    label: '创建网络情景',
    requiredPermissions: ['control.ai.network.manage'],
  },
  {
    allowedStatuses: ['DRAFT', 'FAILED'],
    id: 'runScenario',
    label: '运行网络模拟',
    requiredPermissions: ['control.ai.network.run'],
  },
  {
    allowedStatuses: ['COMPLETED'],
    id: 'exportScenario',
    label: '导出情景比较',
    requiredPermissions: ['control.ai.network.export'],
  },
]);
...
export function AiOptimizationWorkbench() {
  ...
  const selectedForecast = view.forecasts.find(
    ({ id }) => id === selectedForecastIds[0],
  );
  const selectedScenario = view.scenarios.find(
    ({ id }) => id === selectedScenarioIds[0],
  );
  const actionStatus =
    selectedLoad?.status ??
  ...
  const request = useCallback(
    async (path: string, init?: RequestInit) => {
      if (!accessToken || !claims)
        throw new Error('请先登录后使用 AI 优化工作台');
      ...
      const body = (await response.json()) as AiView & {
        code?: string;
        message?: string;
      };
      if (!response.ok)
        throw new Error(
          `${body.code ?? 'REQUEST_FAILED'}: ${body.message ?? 'AI 优化请求失败'}`,
        );
      return body;
    },
    [accessToken, claims],
  ...
  useEffect(() => {
    void refresh().catch((caught: unknown) =>
      setError(caught instanceof Error ? caught.message : 'AI 工作台查询失败'),
    );
  }, [refresh]);
    ...
    try {
      if (actionId === 'route') {
        await post('/api/v1/control/ai/route-optimizations', {
          ...
          vehicles: [
            { capacity: 10, fixedCost: 5, id: 'VEHICLE-1', maxMinutes: 180 },
          ],
        });
        ...
        await post(
          `/api/v1/control/ai/load-optimizations/${selectedLoad.id}/decision`,
          {
            decision: 'CONFIRMED',
            expectedVersion: selectedLoad.version,
            reason: '计划员复核硬约束与卸货顺序后确认',
          },
        );
      } else if (actionId === 'feedbackLoad' && selectedLoad) {
        await post(
          `/api/v1/control/ai/load-optimizations/${selectedLoad.id}/deviations`,
          {
            actual: { movedItems: [], note: '现场装载与建议一致' },
            expectedVersion: selectedLoad.version,
          },
        );
        ...
        await post(
          `/api/v1/control/ai/forecasts/${selectedForecast.id}/publish`,
          { expectedVersion: selectedForecast.version },
        );
        ...
        await post(
          `/api/v1/control/ai/network-scenarios/${selectedScenario.id}/run`,
          { expectedVersion: selectedScenario.version },
        );
      } else if (actionId === 'exportScenario' && selectedScenario) {
        await request(
          `/api/v1/control/ai/network-scenarios/${selectedScenario.id}/export`,
        );
      }
      setNotice(
        '动作已受理；求解任务可通过 jobId 跟踪，结果保留版本、约束与解释。',
      );
      await refresh();
    } catch (caught) {
  ...
  return (
    <section className="ai-optimization-workbench">
      ...
      <Typography.Paragraph>
        路径与装载通过固定 OR-Tools sidecar
        求解；领域层只传约束快照并保存解释。预测、补货与网络情景均版本化，补货建议不会直接修改库存。
      </Typography.Paragraph>
      ...
      <Card title="车辆路径：时间窗、容量、工时与可解释结果">
        <DataGrid
          {...grid}
          columns={[{ key: 'jobRunId', label: '异步 jobId' }, statusColumn]}
          rows={filtered(view.routes)}
          total={filtered(view.routes).length}
        />
        <DataGrid
          {...grid}
          columns={[
            { key: 'solverVersion', label: '求解器版本' },
            { key: 'feasibility', label: '可行性' },
            { key: 'explanation', label: '约束解释' },
          ]}
          rows={view.routeResults}
          total={view.routeResults.length}
        />
      </Card>
      ...
      <Card title="装载建议：人工确认与实际偏差反馈">
        <DataGrid
          ...
          columns={[
            ...
            { key: 'explanation', label: '建议解释' },
            statusColumn,
          ]}
          onSelectionChange={(ids) => setSelectedLoadIds(ids.slice(-1))}
          rows={filtered(view.loads)}
          selectedIds={selectedLoadIds}
          total={filtered(view.loads).length}
        />
      </Card>
      ...
      <Card title="需求预测：训练窗口、版本与置信区间">
        <DataGrid
          {...grid}
          columns={[
            { key: 'versionNumber', label: '预测版本' },
            { key: 'modelVersion', label: '模型版本' },
            { key: 'seriesKey', label: '序列键' },
            statusColumn,
          ]}
          onSelectionChange={(ids) => setSelectedForecastIds(ids.slice(-1))}
          rows={filtered(view.forecasts)}
          selectedIds={selectedForecastIds}
          total={filtered(view.forecasts).length}
        />
      </Card>
      ...
      <Card title="库存策略建议：只建议、不直接改库存">
        <DataGrid
          {...grid}
          columns={[
            { key: 'warehouseId', label: '仓库 ID' },
            { key: 'productId', label: '商品 ID' },
            { key: 'explanation', label: '策略解释' },
            statusColumn,
          ]}
          rows={filtered(view.recommendations)}
          total={filtered(view.recommendations).length}
        />
      </Card>
      ...
      <Card title="供应链网络：独立基线、情景比较与导出">
        <DataGrid
          {...grid}
          columns={[
            { key: 'scenarioNo', label: '情景号' },
            { key: 'name', label: '情景名称' },
            { key: 'jobRunId', label: '异步 jobId' },
            statusColumn,
          ]}
          onSelectionChange={(ids) => setSelectedScenarioIds(ids.slice(-1))}
          rows={filtered(view.scenarios)}
          selectedIds={selectedScenarioIds}
          total={filtered(view.scenarios).length}
        />
        <Typography.Paragraph>
          已固化不可变模拟结果 {view.simulationResults.length} 份。
        </Typography.Paragraph>
      </Card>
    </section>
  );
