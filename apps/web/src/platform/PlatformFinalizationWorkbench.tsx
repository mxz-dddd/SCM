...
export function PlatformFinalizationWorkbench() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const [localeRow, conversionRows, flagRows, templateRows, jobRows] =
        await Promise.all([
          request(`${base}/locale`),
          request(`${base}/unit-conversions`),
          request(`${base}/feature-flags`),
          request(`${base}/print-templates`),
          request(`${base}/print-jobs`),
        ]);
      ...
      setError(caught instanceof Error ? caught.message : '平台数据查询失败');
    }
  ...
  async function loadComments(values?: Record<string, string | undefined>) {
    ...
    try {
      const rows = await request(
        `${base}/comments?businessType=ORDER&businessId=${encodeURIComponent(id)}`,
      );
      setComments(rows as unknown as Row[]);
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : '评论查询失败');
    }
  }
  ...
  return (
    <section className="platform-finalization-workbench">
      ...
      <Typography.Paragraph>
        UTC
        时间按业务时区显示，数量保留原单位与换算版本；渐进发布、协同评论和打印任务均可审计与重放。
      </Typography.Paragraph>
      ...
      <Card title="LocaleContext 与单位换算">
        <Space wrap>
          <Select
            ...
            onChange={(language) => setLocale({ ...locale, language })}
            options={[
          ...
          <Input
            ...
            onChange={(event) =>
              setLocale({ ...locale, timeZone: event.target.value })
            }
          />
          <Button
            ...
            onClick={() =>
              void act(
                'locale',
                { ...locale, expectedVersion: locale.version || undefined },
                '区域设置已保存',
              )
            }
          >
        ...
        {grid(conversions, [
          { key: 'code', label: '代码' },
          { key: 'fromUom', label: '原单位' },
          { key: 'toUom', label: '基础单位' },
          { key: 'factor', label: '换算因子' },
          statusColumn,
        ])}
      </Card>
      ...
      <Card title="评论、@提及与外部可见性">
        <QueryPanel
          fields={[{ label: '业务对象 ID', name: 'businessId', quick: true }]}
          onQuery={(values) => void loadComments(values)}
          onReset={() => {
            setBusinessId('');
            setComments([]);
          }}
        ...
        <Space wrap>
          <Button
            disabled={!accessToken || !businessId}
            onClick={() =>
              void act(
                'comments',
                {
      ...
      <Card title="PrintTemplate、打印机路由与 PrintJob">
          ...
          <Button
            ...
            onClick={() =>
              void act(
                'printers',
                {
                  code: `PRINTER_${Date.now()}`,
                  endpointRef: 'printer://local-label',
                  name: '标签打印机',
                  routeTags: ['LABEL'],
                },
                '打印机已注册',
        ...
        {grid(printJobs, [
          { key: 'documentType', label: '打印类型' },
          { key: 'copies', label: '份数' },
          { key: 'printerId', label: '打印机' },
          statusColumn,
        ])}
      </Card>
    </section>
