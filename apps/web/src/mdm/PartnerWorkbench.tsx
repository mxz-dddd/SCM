...
interface PartnerDetail extends PartnerRow {
  ...
  contacts: ContactRow[];
  roles: RoleRow[];
  serviceZones: ZoneRow[];
}

const registry = createActionRegistry<PartnerRow['status'] | 'NONE'>([
  {
    allowedStatuses: ['DRAFT', 'ACTIVE', 'SUSPENDED'],
    id: 'edit',
    label: '编辑伙伴',
    requiredPermissions: ['mdm.partner.write'],
  },
  {
    allowedStatuses: ['DRAFT', 'SUSPENDED'],
    confirmMessage: '确认启用该伙伴供新业务引用？',
    id: 'ACTIVE',
    label: '启用',
    requiredPermissions: ['mdm.partner.write'],
  },
  ...
  },
  {
    ...
    confirmMessage: '停用不可恢复，历史业务仍保留伙伴快照。',
    id: 'INACTIVE',
    label: '停用',
    requiredPermissions: ['mdm.partner.write'],
  },
...
export function PartnerWorkbench() {
  ...
  const permissions = useMemo(
    () =>
      new Set(
        claims
          ? ['mdm.partner.read', 'mdm.partner.write', 'mdm.partner.geocode']
          : [],
      ),
    [claims],
  );
  ...
  const request = useCallback(
    async (path: string, init?: RequestInit) => {
      if (!accessToken || !claims)
        throw new Error('请先登录后使用伙伴主数据工作台');
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const [partnerRows, correctionRows] = await Promise.all([
        request(`/api/v1/mdm/partners${status ? `?status=${status}` : ''}`),
        request('/api/v1/mdm/partner-addresses/correction-queue'),
      ]);
      setPartners(partnerRows as unknown as PartnerRow[]);
      setCorrections(correctionRows as unknown as AddressRow[]);
      setError(undefined);
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : '伙伴查询失败');
    }
  ...
  const decisions = registry.list().map(({ id }) =>
    registry.decide(id, {
  ...
  async function loadDetail(id: string) {
    try {
      setDetail(
        (await request(
          `/api/v1/mdm/partners/${id}`,
        )) as unknown as PartnerDetail,
      );
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : '伙伴详情查询失败');
    }
  }
  ...
  async function execute(actionId: string) {
    ...
    if (decision.confirmMessage && !window.confirm(decision.confirmMessage))
      return;
    try {
      await request(`/api/v1/mdm/partners/${selected.id}/${actionId}`, {
      ...
      setError(caught instanceof Error ? caught.message : '伙伴动作失败');
    }
  }

  return (
    <section className="partner-workbench">
      <Typography.Title level={2}>伙伴、地址与服务区域</Typography.Title>
      <Typography.Paragraph>
        同一法人可同时承担客户、供应商和承运商角色；地址标准化与地理编码保留原始文本，失败项进入人工校正队列。
      </Typography.Paragraph>
      {notice ? <Alert message={notice} showIcon type="success" /> : null}
      {error ? <Alert message={error} showIcon type="error" /> : null}
      <Card title="统一伙伴档案">
        <QueryPanel
          fields={[{ label: '伙伴状态', name: 'status', quick: true }]}
      ...
      <Drawer
        ...
        open={Boolean(detail)}
        title={detail ? `${detail.code} · ${detail.legalName}` : '伙伴详情'}
        width={900}
      >
        {detail ? (
          <>
            ...
            <Row gutter={[16, 16]}>
              ...
              <Col span={24}>
                <Card size="small" title="地址与地理编码">
                  <DataGrid
                    columns={[
                      { key: 'code', label: '代码' },
                      { key: 'rawText', label: '原始地址' },
                      { key: 'geocodeStatus', label: '地理状态' },
                      { key: 'status', label: '状态' },
                    ]}
                    onPageChange={() => undefined}
                    onSelectionChange={() => undefined}
              ...
              <Col span={12}>
                <Card size="small" title="证照与有效期">
                  <DataGrid
                    columns={[
                      { key: 'certificateType', label: '类型' },
                      { key: 'certificateNo', label: '编号' },
                      { key: 'validUntil', label: '到期日' },
                    ]}
                    onPageChange={() => undefined}
                    onSelectionChange={() => undefined}
                    page={1}
                    ...
                    selectedIds={[]}
                    total={detail.certificates.length}
                  />
                </Card>
              </Col>
              <Col span={12}>
                <Card size="small" title="服务区域">
                  <DataGrid
                    columns={[
                      { key: 'code', label: '代码' },
                      { key: 'name', label: '名称' },
                    ...
                    page={1}
                    pageSize={100}
                    rows={detail.serviceZones}
                    selectedIds={[]}
                    total={detail.serviceZones.length}
                  />
                </Card>
              </Col>
            </Row>
