...
interface OperationsView {
  ...
  deviceCommands: Array<Record<string, unknown> & { id: string; status: string; version: number }>;
  laborAssignments: Array<Record<string, unknown> & { id: string; status: string; version: number }>;
  laborMetrics: Array<Record<string, unknown> & { id: string }>;
  laborStandards: Array<Record<string, unknown> & { id: string }>;
  offlineCommands: Array<Record<string, unknown> & { id: string; status: string }>;
  syncConflicts: Array<Record<string, unknown> & { id: string; status: string; version: number }>;
  valueAddedOrders: Array<Record<string, unknown> & { id: string; status: string; version: number }>;
}

interface DashboardView {
  exceptions: { offlineConflicts: number; weightExceptions: number };
  inventory: { allocatedBase: string; availableBase: string; holdBase: string; onHandBase: string };
...
export function MobileOperationsWorkbench() {
  ...
  const [feedback, setFeedback] = useState<{ message: string; type: 'error' | 'success' | 'warning' }>();
  const deviceId = useMemo(() => `WEB-RF-${claims?.subject ?? 'anonymous'}`, [claims]);
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setFeedback({ message: error instanceof Error ? error.message : '作业台加载失败', type: 'error' });
    }
  ...
  function signal(type: 'error' | 'success' | 'warning', message: string) {
    ...
    if ('vibrate' in navigator) navigator.vibrate(type === 'error' ? [150, 80, 150] : 80);
  }
  ...
  function enqueueBarcode() {
    ...
    if (!businessRef || !Number.isInteger(businessVersion) || businessVersion < 1) {
  ...
  async function syncQueue() {
    ...
    try {
      const result = (await request(`/api/v1/wms/offline-devices/${encodeURIComponent(deviceId)}/sync`, {
      ...
      setQueue([]);
      signal(conflicts ? 'warning' : 'success', conflicts ? `${conflicts} 条命令进入人工冲突处理` : '离线命令已按设备序号同步');
      await refresh();
    } catch (error) {
      signal('error', error instanceof Error ? error.message : '同步失败，队列已保留');
    }
  }
  ...
  async function createVas() {
    ...
    const type = window.prompt('RELABEL / REPACK / KITTING / DISASSEMBLY / ASSEMBLY', 'REPACK');
    ...
    try {
      await request('/api/v1/wms/value-added-orders', {
        body: JSON.stringify({
          ...
          instructionSnapshot: { steps: ['SCAN_SOURCE', 'PROCESS', 'QUALITY_CONFIRM'] },
  ...
  async function advanceVas() {
    const order = operations.valueAddedOrders.find(({ status }) => ['OPEN', 'IN_PROGRESS', 'QUALITY_HOLD'].includes(status));
    if (!order) return;
    try {
      if (order.status === 'OPEN')
        await request(`/api/v1/wms/value-added-orders/${order.id}/transition`, {
          body: JSON.stringify({ expectedVersion: order.version, targetStatus: 'IN_PROGRESS' }),
          method: 'POST',
        });
      else {
        ...
        await request(`/api/v1/wms/value-added-orders/${order.id}/transition`, {
          body: JSON.stringify({
            ...
            outputSnapshot: { handlingUnits: [window.prompt('输出 LPN')], steps: ['DONE'], uom: 'EA' },
  ...
  const largeButtonStyle = { height: 72, fontSize: 20, fontWeight: 700, width: '100%' } as const;
  return (
    <section>
      <Typography.Title level={2}>移动仓库作业与运营看板</Typography.Title>
      <Typography.Paragraph>扫码枪回车即入队；绿色成功、黄色待人工、红色阻止，并用设备振动反馈。</Typography.Paragraph>
      {feedback ? <Alert message={feedback.message} showIcon type={feedback.type} /> : null}
      <Row gutter={[12, 12]}>
        <Col xs={12} md={6}><Card><Statistic title="待拣选" value={dashboard?.tasks.pickOpen ?? 0} /></Card></Col>
        <Col xs={12} md={6}><Card><Statistic title="待包装" value={dashboard?.tasks.packOpen ?? 0} /></Card></Col>
        <Col xs={12} md={6}><Card><Statistic title="待装车" value={dashboard?.tasks.loadOpen ?? 0} /></Card></Col>
        <Col xs={12} md={6}><Card><Statistic title="异常" value={(dashboard?.exceptions.offlineConflicts ?? 0) + (dashboard?.exceptions.weightExceptions ?? 0)} /></Card></Col>
      </Row>
      <Card title="RF / PDA 扫描" style={{ marginTop: 16 }}>
        ...
        <Space direction="vertical" size="middle" style={{ marginTop: 16, width: '100%' }}>
          <Button disabled={!claims} onClick={enqueueBarcode} style={largeButtonStyle} type="primary">确认扫描</Button>
          <Button danger={queue.length > 0} disabled={!claims} onClick={() => void syncQueue()} style={largeButtonStyle}>同步离线队列（{queue.length}）</Button>
        </Space>
      </Card>
      <Row gutter={[12, 12]} style={{ marginTop: 16 }}>
        <Col xs={24} md={8}><Button disabled={!claims} onClick={() => void createVas()} style={largeButtonStyle}>新建增值作业</Button></Col>
        <Col xs={24} md={8}><Button disabled={!claims} onClick={() => void advanceVas()} style={largeButtonStyle}>开始 / 完成作业</Button></Col>
        <Col xs={24} md={8}><Button disabled={!claims} onClick={() => void issuePrint()} style={largeButtonStyle}>发送打印指令</Button></Col>
      </Row>
      <Card title="作业状态" style={{ marginTop: 16 }}>
        <Space wrap>
          ...
          <Tag color="gold">离线冲突 {operations.syncConflicts.filter(({ status }) => status === 'OPEN').length}</Tag>
