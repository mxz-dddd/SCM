...
databaseDescribe('WMS value-added, labor, offline RF and device persistence', () => {
  ...
  it('preserves conservation, immutable facts and offline replay semantics', async () => {
    ...
    const created = await service.createValueAddedOrder(
      {
        ...
        instructionSnapshot: { preserveTrace: true, steps: ['SCAN', 'RELABEL'] },
    ...
    expect(synced.results[0]).toMatchObject({ replayed: false, status: 'APPLIED' });
    ...
    expect(replayed.results[0]).toMatchObject({ replayed: true, status: 'APPLIED' });
