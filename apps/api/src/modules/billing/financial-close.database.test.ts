...
databaseDescribe(
  'Billing invoice, payment, period close and margin report',
  () => {
    ...
    it('prevents over-invoicing, settles partial cash flows and closes audited periods', async () => {
      ...
      const allocation =
        await prisma.billingSettlementAllocation.findFirstOrThrow({
          where: { paymentId: payment1.paymentId },
        });
      await expect(
        prisma.billingSettlementAllocation.delete({
