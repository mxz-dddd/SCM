...
describe('order release authorization', () => {
  it('marks release writes for common Idempotency-Key replay', () => {
    ...
    expect(
      reflector.get(
        IDEMPOTENCY_SCOPE,
        FulfillmentReleaseController.prototype.release,
      ),
    ).toBe('oms.order.release.v1');
    expect(
      reflector.get(
        IDEMPOTENCY_SCOPE,
        FulfillmentReleaseController.prototype.batch,
      ),
    ).toBe('oms.order.release-batch.v1');
  });
  it('denies batch release without its dedicated permission', async () => {
    ...
    const request = {
      header: () => 'release-test',
      originalUrl: '/api/v1/oms/order-release-batches',
      tenantContext: {
    ...
    const execution = {
      getClass: () => FulfillmentReleaseController,
      getHandler: () => FulfillmentReleaseController.prototype.batch,
    ...
    expect(decide).toHaveBeenCalledWith(
      expect.objectContaining({ permissionCode: 'oms.order.release.batch' }),
    );
  });
});
