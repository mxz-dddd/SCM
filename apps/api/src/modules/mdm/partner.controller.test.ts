...
describe('partner geocode authorization', () => {
  it('denies geocode correction without mdm.partner.geocode', async () => {
    ...
    const request = {
      header: (name: string) =>
        name === 'X-Correlation-Id' ? 'partner-permission-test' : undefined,
      originalUrl: '/api/v1/mdm/partner-addresses/address-1/geocode',
      tenantContext: {
    ...
    const executionContext = {
      ...
      getHandler: () => PartnerController.prototype.geocode,
    ...
    expect(decide).toHaveBeenCalledWith(
      expect.objectContaining({ permissionCode: 'mdm.partner.geocode' }),
    );
  });
});
