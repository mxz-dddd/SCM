...
describe('warehouse projection authorization', () => {
  it('denies projection writes without mdm.warehouse.project', async () => {
    ...
    const request = {
      header: (name: string) =>
        name === 'X-Correlation-Id' ? 'warehouse-permission-test' : undefined,
      originalUrl: '/api/v1/mdm/warehouses/id/usage-projection',
      tenantContext: {
    ...
    const executionContext = {
      getClass: () => WarehouseFleetController,
      getHandler: () => WarehouseFleetController.prototype.updateUsage,
    ...
    expect(decide).toHaveBeenCalledWith(
      expect.objectContaining({ permissionCode: 'mdm.warehouse.project' }),
    );
  });
});
