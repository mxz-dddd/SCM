...
describe('product command authorization', () => {
  it('denies publishing when RBAC does not grant mdm.product.publish', async () => {
    ...
    const request = {
      header: (name: string) =>
        name === 'X-Correlation-Id' ? 'mdm-permission-test' : undefined,
      originalUrl:
        '/api/v1/mdm/products/10000000-0000-4000-8000-000000000001/publish',
      tenantContext: {
    ...
    const executionContext = {
      ...
      getHandler: () => ProductController.prototype.publishProduct,
    ...
    expect(decide).toHaveBeenCalledWith(
      expect.objectContaining({ permissionCode: 'mdm.product.publish' }),
    );
  });
});
