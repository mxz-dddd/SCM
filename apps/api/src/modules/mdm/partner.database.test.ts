...
databaseDescribe('partner, address and service zone persistence', () => {
  ...
  it('preserves raw addresses and external-code history with tenant isolation and idempotency', async () => {
    ...
    const context: TenantContext = {
      ...
      deviceId: 'partner-database-test',
      organizationIds: [],
      permissionVersion: 1,
      tenantId,
      tokenId: randomUUID(),
    ...
    try {
      const input = {
        ...
        legalName: '测试供应链有限公司',
        serviceCapabilities: { coldChain: true },
        taxId: 'TAX-001',
      };
      ...
      await expect(
        service.save(undefined, input, context, command(key)),
      ...
      const customerRole = await service.addRole(
        {
      ...
      const certificate = await service.addCertificate(
        {
      ...
      const expiredCertificate = await service.addCertificate(
        {
          certificateNo: 'CERT-OLD',
          certificateType: 'CARRIER_LICENSE',
          partnerId: created.partnerId,
      ...
      await expect(
        service.expireCertificate(
          expiredCertificate.certificateId,
          { expectedVersion: expiredCertificate.version },
          context,
          command(),
        ),
      ).resolves.toMatchObject({ status: 'EXPIRED' });
      const address = await service.saveAddress(
        {
          addressType: 'SHIP_TO',
          city: '上海市',
          code: 'SH-01',
          countryCode: 'CN',
          partnerId: created.partnerId,
          rawText: '上海市浦东新区原始地址 88 号',
          timeWindowFrom: '09:00',
          timeWindowUntil: '18:00',
        },
        context,
        command(),
      );
      const failure = await service.geocode(
        address.addressId,
        {
      ...
      expect(
        (await service.listCorrectionQueue(context)).map(({ id }) => id),
      ).toContain(address.addressId);
      const corrected = await service.geocode(
        address.addressId,
        {
          city: '上海市',
          ...
          provider: 'MANUAL',
          targetStatus: 'MANUAL_CORRECTION',
        },
        context,
        command(),
      );
      ...
      await expect(
        prisma.partnerAddress.update({
          data: { rawText: '被覆盖' },
          where: { id: address.addressId },
        }),
      ).rejects.toThrow();

      const zone = await service.saveServiceZone(
        {
          code: `ZONE-${randomUUID().slice(0, 6).toUpperCase()}`,
          name: '华东服务区',
          partnerId: created.partnerId,
          postalPrefixes: ['20', '21'],
          zoneType: 'POSTAL_PREFIX',
        },
        context,
        command(),
      );
      const activeZone = await service.transitionServiceZone(
        zone.serviceZoneId,
        'ACTIVE',
      ...
      await service.transitionServiceZone(
        zone.serviceZoneId,
        'INACTIVE',
        { expectedVersion: activeZone.version },
        context,
        command(),
      );

      const mapping1 = await service.saveExternalCode(
        {
          externalCode: 'ERP-CUSTOMER-001',
          objectId: created.partnerId,
          objectType: 'PARTNER',
          sourceSystem: 'ERP',
        },
        context,
      ...
      const mapping2 = await service.saveExternalCode(
        {
          externalCode: 'ERP-CUSTOMER-002',
          objectId: created.partnerId,
          objectType: 'PARTNER',
          sourceSystem: 'ERP',
        },
      ...
      await expect(
        prisma.externalCodeMap.update({
          data: { externalCode: 'MUTATED' },
          where: { id: mapping1.externalCodeMapId },
        }),
      ).rejects.toThrow();
      await expect(service.get(created.partnerId, other)).rejects.toMatchObject(
        { code: 'PARTNER_NOT_FOUND', statusCode: 404 },
      );
      ...
      const suspended = await service.transition(
        created.partnerId,
        'SUSPENDED',
      ...
      const resumed = await service.transition(
        created.partnerId,
        'ACTIVE',
      ...
      const inactive = await service.transition(
        created.partnerId,
        'INACTIVE',
      ...
      await expect(
        ...
        code: 'PARTNER_TRANSITION_INVALID',
        statusCode: 409,
      });
      ...
      await service
        .deactivateChild(
          'role',
          customerRole.partnerRoleId,
          { expectedVersion: customerRole.version },
          context,
          command(),
        )
        .catch((error: unknown) => {
      ...
      expect(
        await prisma.platformOutbox.count({
          where: { aggregateType: 'Partner', tenantId },
        }),
