...
const event = (
  ...
  occurredAt: new Date(
    Date.UTC(2026, 6, 15, 10, aggregateVersion),
  ).toISOString(),
  payload,
  schemaVersion: 1,
  traceId: randomUUID(),
...
databaseDescribe('Customer mobile and scoped partner portal', () => {
  ...
  beforeAll(() => {
    ...
    service = new MobilePortalService(
      ...
      new EventConsumptionFacade(
        new EventService(idempotency, prisma as never),
      ),
    );
  });
  ...
  it('projects current customer views, ignores stale versions and preserves immutable history', async () => {
    ...
    const firstEvent = event(aggregateId, 1, {
      orderNo: 'SO-1',
      status: 'OPEN',
    });
    ...
    expect(portal.projections).toContainEqual(
    ...
    const projection =
      await prisma.integrationPortalProjection.findFirstOrThrow({
        where: { sourceEventId: firstEvent.eventId },
      });
    await expect(
      prisma.integrationPortalProjection.update({
        data: { snapshot: { status: 'TAMPERED' } },
        where: { id: projection.id },
      }),
    ).rejects.toBeDefined();
    const disabled = await service.transitionGrant(
      grant.grantId,
      { expectedVersion: grant.version, target: 'INACTIVE' },
      administrator,
      command(),
    );
    await expect(service.portal(customer)).rejects.toMatchObject({
      code: 'PORTAL_SCOPE_DENIED',
      statusCode: 403,
    });
    await service.transitionGrant(
      grant.grantId,
      { expectedVersion: disabled.version, target: 'ACTIVE' },
      administrator,
      command(),
    );
    await expect(
  ...
  it('routes customer commands through Outbox and governs acknowledged and failed terminal paths', async () => {
    ...
    await service.project(
      {
        ...
        projectionKey: 'SO-2',
        projectionType: 'ORDER',
        snapshot: { orderNo: 'SO-2', status: 'OPEN' },
      },
      administrator,
      command(),
    );
    ...
    const outbox = await prisma.platformOutbox.findFirstOrThrow({
      where: {
        aggregateId: accepted.portalCommandId,
        eventName: 'integration.portal-command-requested.v1',
      },
    });
    ...
    const dispatched = await service.dispatchCommand(
      accepted.portalCommandId,
      { expectedVersion: 1 },
      administrator,
      command(),
    );
    await expect(
      service.completeCommand(
        accepted.portalCommandId,
        { expectedVersion: dispatched.version, outcome: 'ACKNOWLEDGED' },
        administrator,
        command(),
      ),
    ...
    const cancellationDispatched = await service.dispatchCommand(
      cancellation.portalCommandId,
      { expectedVersion: 1 },
      administrator,
      command(),
    );
    await expect(
      service.completeCommand(
        cancellation.portalCommandId,
        {
          errorCode: 'ORDER_ALREADY_EXECUTING',
          expectedVersion: cancellationDispatched.version,
          outcome: 'FAILED',
        },
        administrator,
        command(),
  ...
  it('isolates supplier and carrier projections and commands by exact partner scope', async () => {
    ...
    await service.saveGrant(
      {
        ...
        displayName: 'Carrier',
        permissions: ['VIEW_ALL', 'COMMAND_ALL'],
        principalRef: carrierRef,
        principalType: 'CARRIER',
      },
      administrator,
      command(),
    );
    ...
    await service.project(
      {
        ...
        principalType: 'CARRIER',
        projectionKey: 'SHP-1',
        projectionType: 'TENDER',
        snapshot: { shipmentNo: 'SHP-1' },
      },
      administrator,
      command(),
    );
    const supplierPortal = await service.portal(
      context(tenantId, supplierAccount),
    );
    expect(supplierPortal.projections).toHaveLength(1);
    expect(supplierPortal.projections[0]).toMatchObject({
      businessRef: 'PO-1',
      principalRef: supplierRef,
    });
    await expect(
      service.createCommand(
        {
          businessRef: 'SHP-1',
          commandType: 'SUPPLIER_ASN_SUBMIT',
    ...
    await expect(
      service.createCommand(
        {
          businessRef: 'SHP-1',
          commandType: 'CARRIER_TENDER_RESPONSE',
