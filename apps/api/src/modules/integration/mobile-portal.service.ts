...
const PROJECTION_PERMISSIONS: Readonly<
  Record<IntegrationPortalProjectionType, string>
> = {
...
> = {
  ATTACHMENT_ADD: {
    ...
    principals: ['CUSTOMER', 'SUPPLIER', 'CARRIER'],
    targetDomain: 'platform',
  },
  CARRIER_POD_SUBMIT: {
    permission: 'POD_SUBMIT',
    principals: ['CARRIER'],
    targetDomain: 'tms',
  },
  CARRIER_RECONCILIATION_RESPONSE: {
    permission: 'RECONCILIATION_RESPOND',
    principals: ['CARRIER'],
  ...
  CARRIER_TENDER_RESPONSE: {
    permission: 'TENDER_RESPOND',
    principals: ['CARRIER'],
    targetDomain: 'tms',
  },
  CARRIER_TRACKING_REPORT: {
    permission: 'TRACKING_REPORT',
    principals: ['CARRIER'],
    targetDomain: 'tms',
  },
  CUSTOMER_CANCEL: {
  ...
  SUPPLIER_ASN_SUBMIT: {
    permission: 'ASN_SUBMIT',
    principals: ['SUPPLIER'],
    targetDomain: 'oms',
  },
  SUPPLIER_ORDER_RESPONSE: {
    permission: 'ORDER_RESPOND',
    principals: ['SUPPLIER'],
    targetDomain: 'oms',
  },
};
...
const PRINCIPAL_PROJECTIONS: Readonly<
  Record<
    IntegrationPortalPrincipalType,
    readonly IntegrationPortalProjectionType[]
  >
> = {
  CARRIER: [
    'TENDER',
...
export class MobilePortalService {
  ...
  async administration(context: TenantContext) {
    ...
    const [grants, projections, commands] = await Promise.all([
      this.prisma.integrationPortalAccessGrant.findMany({
        orderBy: { updatedAt: 'desc' },
        where,
      }),
      this.prisma.integrationPortalProjection.findMany({
        orderBy: { occurredAt: 'desc' },
        take: 300,
        where,
      }),
      this.prisma.integrationPortalCommand.findMany({
  ...
  async portal(
    ...
    if (!grants.length)
      throw new AppError(
        'PORTAL_SCOPE_DENIED',
        'No active customer or partner scope is assigned',
        403,
      );
    const rows = await this.prisma.integrationPortalProjection.findMany({
      ...
      take: 1000,
      where: {
        OR: grants.map(({ principalRef, principalType }) => ({
          principalRef,
          principalType,
        })),
        tenantId: context.tenantId,
    ...
    const allowed = new Map(
      grants.map((grant) => [
        `${grant.principalType}:${grant.principalRef}`,
        new Set(values(grant.permissions)),
      ]),
    );
    const latest = new Map<string, (typeof rows)[number]>();
    for (const row of rows) {
      const permissions = allowed.get(
        `${row.principalType}:${row.principalRef}`,
      );
      if (
        !permissions ||
        (!permissions.has('VIEW_ALL') &&
          !permissions.has(PROJECTION_PERMISSIONS[row.projectionType]))
      )
        continue;
    ...
    const projections = [...latest.values()].filter(
      (row) =>
        !query ||
        `${row.businessRef} ${row.projectionKey} ${JSON.stringify(row.snapshot)}`
          .toLowerCase()
          .includes(query),
    );
    ...
    return {
      commands,
      grants: grants.map(
        ({ displayName, id, permissions, principalRef, principalType }) => ({
          displayName,
          id,
          permissions,
          principalRef,
  ...
  saveGrant(
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      await this.record(
        ...
        grant.id,
        grant.version,
        'integration.portal-access-granted.v1',
        context,
        metadata,
        {
  ...
  transitionGrant(
    ...
    input: {
      ...
      readonly target: 'ACTIVE' | 'INACTIVE';
    },
    context: TenantContext,
    metadata: CommandMetadata,
  ) {
    return this.prisma.$transaction(async (tx) => {
      const grant = await tx.integrationPortalAccessGrant.findFirst({
        where: { id, tenantId: context.tenantId },
      });
      if (!grant)
        this.notFound('PORTAL_GRANT_NOT_FOUND', 'Portal access grant');
      this.version(
        grant.version,
        input.expectedVersion,
        'PORTAL_GRANT_VERSION_CONFLICT',
      );
      if (grant.status === input.target)
        this.transition('PORTAL_GRANT_TRANSITION_INVALID', grant.status);
      const changed = await tx.integrationPortalAccessGrant.update({
        data: {
          status: input.target,
          updatedBy: context.accountId,
          version: { increment: 1 },
      ...
      await this.record(
        ...
        id,
        changed.version,
        `integration.portal-access-${input.target.toLowerCase()}.v1`,
        context,
        metadata,
        {
          accountId: grant.accountId,
  ...
  async project(
    input: ProjectPortalEventInput,
    context: TenantContext,
    metadata: CommandMetadata,
  ) {
    this.principal(input.principalType, input.principalRef);
    if (
      !PRINCIPAL_PROJECTIONS[input.principalType].includes(input.projectionType)
    )
      this.invalid('Projection type is not allowed for the principal');
    ...
    return this.eventConsumption.consumePortalProjection(
      input.event,
      context,
      metadata,
      async (event, tx) => {
  ...
  createCommand(
    ...
    if (
      !specification ||
      !specification.principals.includes(input.principalType)
    )
      this.invalid('Portal command is not allowed for the principal');
    this.principal(input.principalType, input.principalRef);
    this.text(input.businessRef, 'businessRef', 200);
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (
        !grant ||
        grant.status !== 'ACTIVE' ||
        (!values(grant.permissions).includes('COMMAND_ALL') &&
          !values(grant.permissions).includes(specification.permission))
      )
        throw new AppError(
          'PORTAL_SCOPE_DENIED',
          'Portal command is outside the assigned principal scope',
          403,
        );
      const visible = await tx.integrationPortalProjection.count({
        where: {
          businessRef: input.businessRef.trim(),
          principalRef: grant.principalRef,
          principalType: grant.principalType,
      ...
      if (!visible)
        throw new AppError(
          'PORTAL_BUSINESS_SCOPE_DENIED',
          'Business reference is not visible in this portal scope',
          403,
        );
      ...
      await this.record(
        ...
        'integration.portal-command-requested.v1',
        context,
        metadata,
        {
      ...
      return {
        accepted: true,
        portalCommandId: portalCommand.id,
  ...
  completeCommand(
    ...
    if (input.outcome === 'FAILED' && !input.errorCode?.trim())
      this.invalid('errorCode is required for failed portal commands');
    return this.commandTransition(
      id,
      input.expectedVersion,
      input.outcome,
  ...
  private commandTransition(
    ...
    target: 'ACKNOWLEDGED' | 'DISPATCHED' | 'FAILED',
    outcome:
    ...
    return this.prisma.$transaction(async (tx) => {
      const command = await tx.integrationPortalCommand.findFirst({
        where: { id, tenantId: context.tenantId },
      });
      ...
      this.version(
        command.version,
        expectedVersion,
        'PORTAL_COMMAND_VERSION_CONFLICT',
      );
      const allowed =
      ...
      if (!allowed.includes(target))
        this.transition('PORTAL_COMMAND_TRANSITION_INVALID', command.status);
      const changed = await tx.integrationPortalCommand.update({
        data: {
          ...
          errorCode:
            target === 'FAILED' ? (outcome?.errorCode?.trim() ?? null) : null,
          errorMessage:
            target === 'FAILED'
              ? (outcome?.errorMessage?.trim() ?? null)
              : null,
          status: target,
          updatedBy: context.accountId,
          version: { increment: 1 },
      ...
      await this.record(
        ...
        id,
        changed.version,
        `integration.portal-command-${target.toLowerCase()}.v1`,
        context,
        metadata,
        {
  ...
  private commandPayload(type: PortalCommandType, payload: JsonObject): void {
    ...
    if (!Object.keys(payload).length)
      this.invalid('Portal command payload is required');
    if (type === 'CUSTOMER_CANCEL' && !String(payload.reason ?? '').trim())
      this.invalid('Cancellation reason is required');
    if (type === 'CUSTOMER_RETURN' && !Array.isArray(payload.lines))
      this.invalid('Return lines are required');
    if (type === 'MESSAGE_SEND' && !String(payload.message ?? '').trim())
      this.invalid('Message content is required');
    if (type === 'ATTACHMENT_ADD')
      this.uuid(
        String(payload.attachmentId ?? ''),
        'PORTAL_ATTACHMENT_INVALID',
      );
    if (type === 'SUPPLIER_ASN_SUBMIT' && !Array.isArray(payload.lines))
      this.invalid('ASN lines are required');
    if (
      type === 'CARRIER_TRACKING_REPORT' &&
      !String(payload.occurredAt ?? '').trim()
    )
      this.invalid('Tracking event time is required');
    if (type === 'CARRIER_POD_SUBMIT')
      this.uuid(
        String(payload.attachmentId ?? ''),
        'PORTAL_ATTACHMENT_INVALID',
      );
  ...
  private safeSnapshot(value: unknown): void {
    if (Array.isArray(value))
      return value.forEach((item) => this.safeSnapshot(item));
    if (!value || typeof value !== 'object') return;
    for (const [key, child] of Object.entries(
      value as Record<string, unknown>,
    )) {
      if (
        /password|secret|credential|accessToken|refreshToken|internalCost|preciseLocation/i.test(
          key,
        )
      )
        throw new AppError(
          'PORTAL_SENSITIVE_FIELD_REJECTED',
          `Sensitive field ${key} cannot enter a portal snapshot`,
          400,
        );
  ...
  private permissions(input: readonly string[]): void {
    const allowed = new Set([
      'VIEW_ALL',
      'COMMAND_ALL',
      ...Object.values(PROJECTION_PERMISSIONS),
      ...Object.values(COMMANDS).map(({ permission }) => permission),
    ]);
    if (!input.length || input.some((permission) => !allowed.has(permission)))
      this.invalid('Portal permissions are invalid');
  }
  ...
  private principal(
    ...
    if (!PRINCIPAL_PROJECTIONS[type])
      this.invalid('Portal principal type is invalid');
  ...
  private version(actual: number, expected: number, code: string): void {
    if (!Number.isSafeInteger(expected) || actual !== expected)
      throw new AppError(code, 'Portal aggregate version conflict', 409);
  }
