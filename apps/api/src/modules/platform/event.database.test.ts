...
databaseDescribe('outbox ordering, relay leases and inbox deduplication', () => {
  ...
  it('claims one version per aggregate and consumes each event once', async () => {
    ...
    const createOutbox = (id: string, targetAggregateId: string, version: number, maxAttempts = 8) =>
    ...
    try {
      const claims = await Promise.all([
        service.claim({ leaseOwner: 'relay-a', limit: 20 }, context, metadata()),
        service.claim({ leaseOwner: 'relay-b', limit: 20 }, context, metadata()),
      ]);
      ...
      const firstLeaseOwner = claims.find(({ events }) => events.some(({ eventId }) => eventId === event1Id))!.leaseOwner;
      ...
      const next = await service.claim({ leaseOwner: 'relay-next', limit: 20 }, context, metadata());
      ...
      const consumed = await service.consume(
        { consumer: 'control.projection', event: businessEvent },
        context,
        metadata(),
        handler,
      );
      ...
      await expect(
        service.consume(
          { consumer: 'control.projection', event: retryEvent },
          context,
          metadata(),
          failedHandler,
        ),
      ).rejects.toMatchObject({ code: 'EVENT_HANDLER_FAILED', statusCode: 500 });
      ...
      const deadClaim = await service.claim({ leaseOwner: 'relay-dead', limit: 20 }, context, metadata());
      const dead = deadClaim.events.find(({ eventId }) => eventId === deadEventId)!;
      const failed = await service.fail(
        dead.eventId,
        { error: 'broker down', expectedVersion: dead.version, leaseOwner: 'relay-dead' },
        context,
        metadata(),
      );
      expect(failed.status).toBe('DEAD_LETTER');
      const restored = await service.replay(dead.eventId, failed.version, context, metadata());
      expect(restored.status).toBe('PENDING');
    } finally {
      await prisma.$executeRawUnsafe('ALTER TABLE "platform"."event_inbox" DISABLE TRIGGER event_inbox_terminal_immutable');
      await prisma.eventInbox.deleteMany({ where: { tenantId } });
      await prisma.$executeRawUnsafe('ALTER TABLE "platform"."event_inbox" ENABLE TRIGGER event_inbox_terminal_immutable');
