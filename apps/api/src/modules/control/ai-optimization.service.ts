...
const ROUTE_TRANSITIONS: Readonly<
  Record<
    ControlRouteOptimizationStatus,
    readonly ControlRouteOptimizationStatus[]
  >
> = {
  FAILED: ['RUNNING'],
...
const LOAD_TRANSITIONS: Readonly<
  Record<
    ControlLoadOptimizationStatus,
    readonly ControlLoadOptimizationStatus[]
  >
> = {
  CONFIRMED: [],
  FAILED: ['RUNNING'],
...
export class AiOptimizationService {
  ...
  async createRouteOptimization(
    ...
    await this.prisma.$transaction(async (tx) => {
      await tx.controlRouteOptimization.create({
        data: {
          ...
          objectiveSnapshot: json(
            input.objective ?? { mode: 'MIN_DISTANCE_AND_VEHICLES' },
          ),
      ...
      await this.record(
        ...
        'RouteOptimization',
        1,
        'control.route-optimization-queued.v1',
        { status: 'QUEUED' },
        context,
        metadata,
      );
  ...
  async createLoadOptimization(
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      await this.record(
        ...
        1,
        'control.load-optimization-queued.v1',
        { status: 'QUEUED' },
        context,
        metadata,
      );
  ...
  async executeJob(
    ...
    if (
      ...
      !isUuid(input.jobRunId) ||
      !['LOAD', 'NETWORK', 'ROUTE'].includes(input.kind)
    )
  ...
  async decideLoad(
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      await this.record(
        ...
        'LoadOptimizationResult',
        changed.version,
        `control.load-optimization-${input.decision.toLowerCase()}.v1`,
        { reason, status: changed.status },
        context,
        metadata,
      );
  ...
  async recordLoadDeviation(
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (current.status !== 'CONFIRMED')
        this.conflict(
          'Only confirmed load suggestions accept deviation feedback',
        );
      if (current.actualDeviationSnapshot)
        this.conflict('Load deviation feedback is immutable once recorded');
      const changed = await tx.controlLoadOptimizationResult.update({
        data: {
      ...
      await this.record(
        ...
        'LoadOptimizationResult',
        changed.version,
        'control.load-deviation-recorded.v1',
        { actual: input.actual, status: changed.status },
        context,
        metadata,
      );
      return toHttpJson(changed);
  ...
  async createForecast(
    ...
    if (trainingTo <= trainingFrom)
      this.invalid('Forecast training window is invalid');
    if (
    ...
    if (!['DAY', 'WEEK', 'MONTH'].includes(input.granularity))
      this.invalid('granularity is invalid');
    const z = this.zScore(input.confidenceLevel);
    const dimension = this.forecastDimensions(
      input.dimensions,
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      await this.record(
        ...
        'DemandForecast',
        forecast.version,
        'control.demand-forecast-created.v1',
        {
  ...
  async publishForecast(
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      for (const previous of active) {
        ...
        const retired = await tx.controlDemandForecast.update({
          data: {
            status: 'RETIRED',
            updatedBy: context.accountId,
            version: { increment: 1 },
        ...
        await this.record(
          ...
          'DemandForecast',
          retired.version,
          'control.demand-forecast-retired.v1',
          { replacedById: id, status: retired.status },
          context,
          metadata,
        );
      }
      ...
      await this.record(
        ...
        'DemandForecast',
        changed.version,
        'control.demand-forecast-published.v1',
        { status: changed.status, versionNumber: changed.versionNumber },
        context,
        metadata,
      );
      return toHttpJson(changed);
  ...
  async retireForecast(
    ...
    return this.changeForecastStatus(
      id,
  ...
  async recordForecastDeviation(
    ...
    if (
      !Array.isArray(input.actualPoints) ||
      input.actualPoints.length < 1 ||
      input.actualPoints.length > 365
    )
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (forecast.status !== 'PUBLISHED')
        this.conflict(
          'Only published forecasts accept actual deviation feedback',
        );
      const changed = await tx.controlDemandForecast.update({
        data: {
          actualDeviationSnapshot: json(input.actualPoints),
          updatedBy: context.accountId,
          version: { increment: 1 },
        },
        where: { id },
      ...
      await this.record(
        ...
        'DemandForecast',
        changed.version,
        'control.demand-forecast-deviation-recorded.v1',
        { pointCount: input.actualPoints.length, status: changed.status },
        context,
        metadata,
      );
      return toHttpJson(changed);
  ...
  async createInventoryRecommendation(
    ...
    this.scope(input.organizationRef, context);
    if (
      !isUuid(input.forecastId) ||
      !isUuid(input.productId) ||
      !isUuid(input.warehouseId)
    )
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const recommendation =
        await tx.controlInventoryPolicyRecommendation.create({
          data: {
            createdBy: context.accountId,
            expiresAt,
      ...
      await this.record(
        ...
        'control.inventory-policy-recommendation-created.v1',
        { forecastId: input.forecastId, policy, status: recommendation.status },
        context,
        metadata,
      );
  ...
  async decideRecommendation(
    id: string,
    input: {
      decision: 'ACCEPTED' | 'EXPIRED' | 'REJECTED';
      expectedVersion: number;
      reason: string;
    },
    context: TenantContext,
    ...
    const reason = this.text(input.reason, 'reason', 1000);
    return this.prisma.$transaction(async (tx) => {
      const current = await tx.controlInventoryPolicyRecommendation.findFirst({
      ...
      await this.record(
        ...
        'InventoryPolicyRecommendation',
        changed.version,
        `control.inventory-policy-recommendation-${input.decision.toLowerCase()}.v1`,
        { policy: changed.policySnapshot, reason, status: changed.status },
        context,
        metadata,
      );
      return toHttpJson(changed);
  ...
  async createScenario(
    ...
    if (input.baselineScenarioId && !isUuid(input.baselineScenarioId))
      this.invalid('baselineScenarioId is invalid');
    return this.prisma.$transaction(async (tx) => {
      if (input.baselineScenarioId) {
        const baseline = await tx.controlNetworkScenario.findFirst({
          where: {
            id: input.baselineScenarioId,
            status: 'COMPLETED',
            tenantId: context.tenantId,
          },
        });
        if (!baseline)
          this.conflict(
            'Baseline scenario must be completed and tenant-scoped',
          );
      }
      ...
      await this.record(
        ...
        'NetworkScenario',
        scenario.version,
        'control.network-scenario-created.v1',
        { scenarioNo, status: scenario.status },
        context,
        metadata,
      );
  ...
  async runScenario(
    ...
    const queued = await this.prisma.$transaction(async (tx) => {
      ...
      const changed = await tx.controlNetworkScenario.update({
        data: {
          failureCode: null,
          status: 'QUEUED',
          updatedBy: context.accountId,
      ...
      await this.record(
        ...
        'NetworkScenario',
        changed.version,
        'control.network-scenario-queued.v1',
  ...
  async archiveScenario(
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      await this.record(
        ...
        'NetworkScenario',
        changed.version,
        'control.network-scenario-archived.v1',
        { status: changed.status },
        context,
        metadata,
      );
  ...
  async exportScenario(id: string, context: TenantContext) {
    const result = await this.prisma.controlSimulationResult.findFirst({
      where: { scenarioId: id, tenantId: context.tenantId },
    });
    if (!result) this.notFound('Simulation result');
    return toHttpJson({
      export: result.exportSnapshot,
      resultId: result.id,
      scenarioId: id,
    });
  }
  ...
  async workbench(context: TenantContext) {
    const [
      ...
      this.prisma.controlRouteOptimization.findMany({
        orderBy: [{ createdAt: 'desc' }, { id: 'desc' }],
        take: 100,
        where: { tenantId: context.tenantId },
      }),
      this.prisma.controlRouteOptimizationResult.findMany({
        orderBy: [{ createdAt: 'desc' }, { id: 'desc' }],
        take: 100,
        where: { tenantId: context.tenantId },
      }),
      this.prisma.controlLoadOptimizationResult.findMany({
        orderBy: [{ createdAt: 'desc' }, { id: 'desc' }],
        take: 100,
        where: { tenantId: context.tenantId },
      }),
      this.prisma.controlDemandForecast.findMany({
        orderBy: [{ createdAt: 'desc' }, { id: 'desc' }],
        take: 100,
        where: { tenantId: context.tenantId },
      }),
      this.prisma.controlInventoryPolicyRecommendation.findMany({
        orderBy: [{ createdAt: 'desc' }, { id: 'desc' }],
        take: 100,
        where: { tenantId: context.tenantId },
      }),
      this.prisma.controlNetworkScenario.findMany({
        orderBy: [{ createdAt: 'desc' }, { id: 'desc' }],
        take: 100,
        where: { tenantId: context.tenantId },
      }),
      this.prisma.controlSimulationResult.findMany({
        orderBy: [{ createdAt: 'desc' }, { id: 'desc' }],
        take: 100,
        where: { tenantId: context.tenantId },
      }),
  ...
  private async enqueue(
    ...
    try {
      const run = await this.scheduling.enqueueAiOptimization(
        { aggregateId, kind },
        context,
        metadata,
      );
      await this.prisma.$transaction(async (tx) => {
        if (kind === 'ROUTE') {
          const current = await tx.controlRouteOptimization.findUniqueOrThrow({
            where: { id: aggregateId },
          });
          const changed = await tx.controlRouteOptimization.update({
            data: {
          ...
          await this.record(
            ...
            'RouteOptimization',
            changed.version,
            'control.route-optimization-job-linked.v1',
            { jobRunId: run.jobRunId, status: current.status },
            context,
            metadata,
          );
        } else if (kind === 'LOAD') {
          ...
          await this.record(
            ...
            'LoadOptimizationResult',
            changed.version,
            'control.load-optimization-job-linked.v1',
            { jobRunId: run.jobRunId, status: changed.status },
            context,
            metadata,
          );
        } else {
          ...
          await this.record(
            ...
            'NetworkScenario',
            changed.version,
            'control.network-scenario-job-linked.v1',
            { jobRunId: run.jobRunId, status: changed.status },
            context,
            metadata,
          );
  ...
  private async executeRoute(
    ...
    const current = await this.prisma.controlRouteOptimization.findFirst({
      where: { id, tenantId: context.tenantId },
    });
    if (!current) this.notFound('Route optimization');
    if (current.status === 'SUCCEEDED') {
      const result = await this.prisma.controlRouteOptimizationResult.findFirst(
        { where: { optimizationId: id, tenantId: context.tenantId } },
      );
    ...
    const started = await this.prisma.$transaction(async (tx) => {
      ...
      if (fresh.status === 'RUNNING' || fresh.status === 'SUCCEEDED')
        return false;
      this.transition(
        ROUTE_TRANSITIONS,
        fresh.status,
      ...
      await this.record(
        ...
        'RouteOptimization',
        changed.version,
        'control.route-optimization-started.v1',
        { jobRunId, status: changed.status },
        context,
        metadata,
      );
      return true;
    ...
    if (!started)
      return { duplicate: true, optimizationId: id, status: 'RUNNING' };
    try {
      const result = await this.optimizer.solveRoutes(
        object(current.inputSnapshot),
      );
      return await this.prisma.$transaction(async (tx) => {
          ...
          data: {
            ...
            objectiveValue:
              ...
              result.objectiveValue === undefined
                ? null
                : new Prisma.Decimal(String(result.objectiveValue)),
            optimizationId: id,
        ...
        await this.record(
          ...
          'RouteOptimization',
          changed.version,
          'control.route-optimization-completed.v1',
          {
  ...
  private async executeLoad(
    ...
    const current = await this.prisma.controlLoadOptimizationResult.findFirst({
      where: { id, tenantId: context.tenantId },
    });
    if (!current) this.notFound('Load optimization');
    if (['PROPOSED', 'CONFIRMED', 'REJECTED'].includes(current.status))
      return {
        duplicate: true,
        loadOptimizationId: id,
    ...
    const started = await this.prisma.$transaction(async (tx) => {
      ...
      if (
        ...
        ['PROPOSED', 'CONFIRMED', 'REJECTED'].includes(fresh.status)
      )
        return false;
      this.transition(
        LOAD_TRANSITIONS,
      ...
      await this.record(
        ...
        'LoadOptimizationResult',
        changed.version,
        'control.load-optimization-started.v1',
        { jobRunId, status: changed.status },
        context,
        metadata,
      );
    ...
    if (!started)
      return { duplicate: true, loadOptimizationId: id, status: 'RUNNING' };
    try {
      const result = await this.optimizer.solveLoad(
        object(current.inputSnapshot),
      );
      return await this.prisma.$transaction(async (tx) => {
        const fresh = await this.load(tx, id, context);
        ...
        await this.record(
          ...
          'LoadOptimizationResult',
          changed.version,
          'control.load-optimization-proposed.v1',
          { feasibility: result.status, status: changed.status },
          context,
          metadata,
        );
  ...
  private async executeScenario(
    ...
    const current = await this.prisma.controlNetworkScenario.findFirst({
      where: { id, tenantId: context.tenantId },
    });
    ...
    const started = await this.prisma.$transaction(async (tx) => {
      ...
      if (fresh.status === 'RUNNING' || fresh.status === 'COMPLETED')
        return false;
      this.transition(
        SCENARIO_TRANSITIONS,
        fresh.status,
      ...
      await this.record(
        ...
        'NetworkScenario',
        changed.version,
        'control.network-scenario-started.v1',
        { jobRunId, status: changed.status },
        context,
        metadata,
      );
    ...
    try {
      const metrics = this.simulate(
        object(current.baselineSnapshot),
        object(current.changeSnapshot),
      );
      const baselineResult = current.baselineScenarioId
        ? await this.prisma.controlSimulationResult.findFirst({
            where: {
              scenarioId: current.baselineScenarioId,
              tenantId: context.tenantId,
            },
          })
        : null;
      const comparison = this.compareSimulation(
        object(baselineResult?.metricSnapshot),
        metrics,
      );
      const exported = {
        comparison,
        ...
        metrics,
        scenario: { id, name: current.name, scenarioNo: current.scenarioNo },
      };
      return await this.prisma.$transaction(async (tx) => {
        const fresh = await this.scenario(tx, id, context);
        this.transition(
        ...
        await this.record(
          ...
          'NetworkScenario',
          changed.version,
          'control.network-scenario-completed.v1',
          { resultId: result.id, status: changed.status },
          context,
          metadata,
        );
        return toHttpJson({
  ...
  private async failScheduling(
    ...
    await this.prisma.$transaction(async (tx) => {
      if (kind === 'ROUTE') {
        ...
        const changed = await tx.controlRouteOptimization.update({
          data: {
            failureCode: 'JOB_SCHEDULING_FAILED',
            status: 'FAILED',
            updatedBy: context.accountId,
            version: { increment: 1 },
          },
        ...
        await this.record(
          ...
          'RouteOptimization',
          changed.version,
          'control.route-optimization-failed.v1',
          { failureCode: changed.failureCode, status: changed.status },
          context,
          metadata,
        );
      } else if (kind === 'LOAD') {
        ...
        await this.record(
          ...
          'LoadOptimizationResult',
          changed.version,
          'control.load-optimization-failed.v1',
          { failureCode: changed.failureCode, status: changed.status },
          context,
          metadata,
        );
        ...
        await this.record(
          ...
          'NetworkScenario',
          changed.version,
          'control.network-scenario-failed.v1',
          { failureCode: changed.failureCode, status: changed.status },
          context,
          metadata,
        );
  ...
  private async failExecution(
    ...
    const failureCode =
      error instanceof AppError ? error.code : 'OPTIMIZATION_EXECUTION_FAILED';
    await this.prisma.$transaction(async (tx) => {
      if (kind === 'ROUTE') {
        const current = await this.route(tx, id, context);
        ...
        await this.record(
          ...
          changed.version,
          'control.route-optimization-failed.v1',
          { failureCode, status: changed.status },
          context,
          metadata,
        );
      } else if (kind === 'LOAD') {
  ...
  private routePayload(input: RouteOptimizationInput): JsonObject {
    ...
    if (
      ...
      (!Number.isInteger(input.timeLimitSeconds) ||
        input.timeLimitSeconds < 1 ||
        input.timeLimitSeconds > 30)
    )
  ...
  private loadPayload(input: LoadOptimizationInput): JsonObject {
    if (
      ...
      input.containers.length < 1 ||
      input.containers.length > 100
    )
  ...
  private forecastDimensions(value: unknown, organizationRef: string) {
    ...
    const customerId = dimensions.customerId
      ? this.text(dimensions.customerId, 'dimensions.customerId', 100)
      : null;
    const region = dimensions.region
      ? this.text(dimensions.region, 'dimensions.region', 100)
      : null;
  ...
  private history(values: readonly JsonObject[], from: Date, to: Date) {
    ...
    const parsed = values
      .map((raw, index) => {
        ...
        if (at < from || at > to)
          this.invalid('Forecast history falls outside the training window');
        const packageSpecVersionId = this.text(
          raw.packageSpecVersionId,
          `history[${index}].packageSpecVersionId`,
          100,
        );
        if (!isUuid(packageSpecVersionId))
          this.invalid('Forecast history packageSpecVersionId is invalid');
        if (packageVersion && packageVersion !== packageSpecVersionId)
          this.invalid(
            'Forecast history package specification versions must be consistent',
          );
        packageVersion = packageSpecVersionId;
        const pointBaseUom = this.text(
          raw.baseUom,
          `history[${index}].baseUom`,
          30,
        );
        if (baseUom && baseUom !== pointBaseUom)
          this.invalid('Forecast history base units must be consistent');
        baseUom = pointBaseUom;
        const pointUom = this.text(raw.uom, `history[${index}].uom`, 30);
        if (uom && uom !== pointUom)
          this.invalid('Forecast history original units must be consistent');
        uom = pointUom;
        const baseQty = this.number(
          raw.baseQty,
          `history[${index}].baseQty`,
          0,
        );
        const qty = this.number(raw.qty, `history[${index}].qty`, 0);
        if ((qty === 0) !== (baseQty === 0))
          this.invalid(
            'Forecast history original and base quantities must both be zero or non-zero',
          );
        return {
          at,
          baseQty,
    ...
    if (
      ...
      this.invalid(
        'Forecast history quantity conversion must match the package version',
      );
    return parsed;
  }
  ...
  private inventorySnapshot(value: JsonObject) {
    ...
    const versionId = this.text(
      value.packageSpecVersionId,
      'currentInventory.packageSpecVersionId',
      100,
    );
    if (!isUuid(versionId))
      this.invalid('currentInventory.packageSpecVersionId is invalid');
    if (value.unitHoldingCost)
      this.money(value.unitHoldingCost, 'currentInventory.unitHoldingCost');
    if (value.unitOrderingCost)
      this.money(value.unitOrderingCost, 'currentInventory.unitOrderingCost');
    if (value.shelfLifeDays !== undefined)
      this.number(value.shelfLifeDays, 'currentInventory.shelfLifeDays', 1);
    this.boundedJson(value, 'currentInventory');
  }

  private networkSnapshot(value: JsonObject) {
    const warehouses = array(value.warehouses);
    if (warehouses.length < 1 || warehouses.length > 100)
      this.invalid('baselineSnapshot.warehouses must contain 1..100 entries');
    let currency: string | undefined;
    for (const [index, raw] of warehouses.entries()) {
      const warehouse = object(raw);
      this.text(warehouse.id, `warehouses[${index}].id`, 100);
      if (typeof warehouse.open !== 'boolean')
        this.invalid('warehouse.open must be boolean');
      this.number(
        warehouse.capacityBaseQty,
        `warehouses[${index}].capacityBaseQty`,
        0,
      );
      const money = this.money(
        warehouse.fixedCost,
        `warehouses[${index}].fixedCost`,
      );
      if (currency && currency !== money.currency)
        this.invalid('Network costs must share one currency');
      currency = money.currency;
      this.number(warehouse.slaMinutes, `warehouses[${index}].slaMinutes`, 1);
    }
    this.number(
      value.carrierCapacityBaseQty,
      'baselineSnapshot.carrierCapacityBaseQty',
      0,
    );
    this.number(value.demandBaseQty, 'baselineSnapshot.demandBaseQty', 0);
    const variable = this.money(
      value.variableCost,
      'baselineSnapshot.variableCost',
    );
    if (currency !== variable.currency)
      this.invalid('Network costs must share one currency');
  ...
  private simulate(baseline: JsonObject, changes: JsonObject) {
    ...
    const warehouses = array(baseline.warehouses).map((raw) => {
      ...
      return {
        capacity: this.number(
          override.capacityBaseQty ?? value.capacityBaseQty,
          'capacityBaseQty',
          0,
        ),
        ...
        open:
          typeof override.open === 'boolean'
            ? override.open
            : Boolean(value.open),
        slaMinutes: this.number(
          override.slaMinutes ?? value.slaMinutes,
          'slaMinutes',
          1,
        ),
      };
    ...
    const demandMultiplier =
      changes.demandMultiplier === undefined
        ? 1
        : this.number(changes.demandMultiplier, 'changes.demandMultiplier', 0);
    const variableMultiplier =
      changes.variableCostMultiplier === undefined
        ? 1
        : this.number(
            changes.variableCostMultiplier,
            'changes.variableCostMultiplier',
            0,
          );
    const carrierDelta =
      changes.carrierCapacityDeltaBaseQty === undefined
        ? 0
        : this.number(
            changes.carrierCapacityDeltaBaseQty,
            'changes.carrierCapacityDeltaBaseQty',
            -1_000_000_000,
          );
    const demand =
      this.number(baseline.demandBaseQty, 'demandBaseQty', 0) *
      demandMultiplier;
    const warehouseCapacity = warehouses
      .filter((item) => item.open)
      .reduce((sum, item) => sum + item.capacity, 0);
    const carrierCapacity = Math.max(
      0,
      this.number(
        baseline.carrierCapacityBaseQty,
        'carrierCapacityBaseQty',
        0,
      ) + carrierDelta,
    );
    ...
    const fixed = warehouses
      .filter((item) => item.open)
      .reduce(
        (sum, item) => sum.plus(item.fixedCost.amount),
        new Prisma.Decimal(0),
      );
    const total = fixed.plus(
      new Prisma.Decimal(variableCost.amount)
        .mul(fulfilled)
        .mul(variableMultiplier),
    );
    const slaTarget =
      changes.slaTargetMinutes === undefined
        ? Number.POSITIVE_INFINITY
        : this.number(changes.slaTargetMinutes, 'changes.slaTargetMinutes', 1);
    const riskyNodes = warehouses.filter(
      (item) => item.open && item.slaMinutes > slaTarget,
    ).length;
    return {
  ...
  private compareSimulation(baseline: JsonObject, current: JsonObject) {
    ...
    if (baselineCost.currency !== currentCost.currency)
      this.invalid('Simulation currencies cannot be compared');
    return {
      baselineAvailable: true,
      costDelta: {
        amount: new Prisma.Decimal(currentCost.amount)
          .minus(baselineCost.amount)
          .toFixed(6),
        currency: currentCost.currency,
      },
      unmetBaseQtyDelta: (
        this.number(current.unmetBaseQty, 'current.unmetBaseQty', 0) -
        this.number(baseline.unmetBaseQty, 'baseline.unmetBaseQty', 0)
      ).toFixed(6),
    };
  }
  ...
  private money(value: unknown, field: string) {
    ...
    if (!/^[A-Z]{3}$/.test(currency))
      this.invalid(`${field}.currency is invalid`);
    ...
    if (
      ...
      value.some(
        (row) =>
          !Array.isArray(row) ||
          row.length !== size ||
          row.some((item) => !Number.isInteger(item) || item < 0),
      )
    )
      this.invalid(
        `${field} must be a non-negative ${size}x${size} integer matrix`,
      );
  }
  ...
  private zScore(value: unknown) {
    ...
    if (!result)
      this.invalid('confidence/service level must be 0.8, 0.9, 0.95 or 0.99');
    return result;
  }
  ...
  private async route(
    ...
    const result = await tx.controlRouteOptimization.findFirst({
      where: { id, tenantId: context.tenantId },
    });
  ...
  private async load(
    ...
    const result = await tx.controlLoadOptimizationResult.findFirst({
      where: { id, tenantId: context.tenantId },
    });
  ...
  private async forecast(
    ...
    const result = await tx.controlDemandForecast.findFirst({
      where: { id, tenantId: context.tenantId },
    });
  ...
  private async scenario(
    ...
    const result = await tx.controlNetworkScenario.findFirst({
      where: { id, tenantId: context.tenantId },
    });
  ...
  private transition<S extends string>(
    ...
    if (!map[current].includes(target))
      this.conflict(
        `${label} transition ${current} -> ${target} is not allowed`,
      );
  }
  ...
  private version(current: number, expected: number) {
    if (!Number.isInteger(expected) || current !== expected)
      throw new AppError(
        'CONTROL_AI_VERSION_CONFLICT',
        'The optimization version is stale',
        409,
        { retryable: true },
      );
  }
  ...
  private scope(organizationRef: unknown, context: TenantContext) {
    ...
    if (
      ...
      !context.organizationIds.includes(reference)
    )
      throw new AppError(
        'CONTROL_AI_SCOPE_DENIED',
        'The organization is outside the caller data scope',
        403,
      );
  }

  private boundedJson(value: unknown, field: string) {
    if (Buffer.byteLength(JSON.stringify(value)) > 2_000_000)
      this.invalid(`${field} exceeds the 2 MB snapshot limit`);
  }
