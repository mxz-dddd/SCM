...
export class OnsiteOperationsService {
  ...
  async assignDock(
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const assignment = await tx.dockAssignment.create({ data: { appointmentId, assignedFrom, assignedUntil, assignmentMode: input.manualDockRef ? 'MANUAL' : 'AUTO', assignmentNo: await businessNumber(this.prisma, 'AMS_DOCK_ASSIGNMENT', context, metadata, `dock-assignment:${appointmentId}`), createdBy: context.accountId, dockRef: selected.id, dockSnapshot: json(selected), id: assignmentId, queueTicketId: ticket.id, requirementSnapshot: json(input.requirements), tenantId: context.tenantId, updatedBy: context.accountId } });
  ...
  async switchDock(
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const assignmentId = randomUUID();
      const replacement = await tx.dockAssignment.create({ data: { appointmentId, assignedFrom: switchedAt, assignedUntil: active.assignedUntil, assignmentMode: 'MANUAL', assignmentNo: await businessNumber(this.prisma, 'AMS_DOCK_ASSIGNMENT', context, metadata, `dock-assignment-switch:${appointmentId}:${active.version}`), createdBy: context.accountId, dockRef: selected.id, dockSnapshot: json(selected), id: assignmentId, queueTicketId: active.queueTicketId, requirementSnapshot: json(input.requirements), tenantId: context.tenantId, updatedBy: context.accountId } });
