...
describe('AMS gate, queue and dock HTTP contract', () => {
  it('uses generic idempotency on every on-site write command', () => {
    for (const method of [
      ...
      expect(
        Reflect.getMetadata(
          IDEMPOTENCY_SCOPE,
          OnsiteOperationsController.prototype[method],
        ),
      ).toMatch(/^ams\./);
  });
  ...
  it('denies dock assignment without dispatcher permission', async () => {
    ...
    const request = {
      header: () => 'onsite-permission-test',
      originalUrl: '/api/v1/ams/onsite/appointments/id/dock-assignments',
      tenantContext: {
    ...
    const execution = {
      getClass: () => OnsiteOperationsController,
      getHandler: () => OnsiteOperationsController.prototype.assignDock,
    ...
    expect(decide).toHaveBeenCalledWith(
      expect.objectContaining({ permissionCode: 'ams.dock.assign' }),
    );
  });
});
