...
export class AppointmentIntakeService {
  ...
  private async recordDecision(tx: Prisma.TransactionClient, appointment: { id: string; status: 'DRAFT' | 'SUBMITTED' | 'PENDING' | 'CONFIRMED' | 'REJECTED' | 'CANCELLED' | 'CHECKED_IN' | 'QUEUED' | 'DOCKED' | 'OPERATING' | 'CHECKED_OUT' | 'COMPLETED' | 'NO_SHOW'; approvalPolicySnapshot: Prisma.JsonValue }, toStatus: 'SUBMITTED' | 'PENDING' | 'CONFIRMED' | 'REJECTED' | 'CANCELLED', decision: string, reason: string, context: TenantContext, suggestedTimeSlotId?: string) {
